...
**specVersion: 0.4.0（draft）** — 通用 Token、产品 Token、规范站内容、生成产物与产品快照已进入同一条校验和同步管线；跨产品组件验收完成后升 1.0。
...
- **0.4.2**（2026-07-28）：排版改为平台默认字体——废止钉死 `.SF Pro Text` 与强制 fallback；壳层标题 w700 + 缓和负字距；内容层沉浸展示仍可 w800。参考实现：开听 `SoundTheme`。
