...
- **内页页头（SettingsBackButton）**：返回钮 + 页面标题（title 档 w600）同一行，页顶 padding 8——压低页头让内容尽快开始，不再用"裸返回箭头 + 大留白"（0.6.3 之前的做法）；
- 分区：`caption` w600 secondary 小节标签（左内缩 4、下间距 10）+ 分组卡片，区间距使用当前 Profile 的 `sectionGap`；
...
皮肤、主题色、播放器样式这类「看一眼就想点」的选择器**在分组卡内直接展示**，不藏在折叠行后面。外观分组为**整块卡**（无行间分隔线），卡内用子块标签（`SettingsBlockLabel`，`caption` w600 secondary、字距 0.2，14/12/14/2 内边距）分隔各选择器。
